...
const navigate= useNavigate()
  ...
  const fectchSessionDetails = async () => {
    try {
      const paymentsQuery = query(collection(db, 'eshsessions'), where('patientId', '==', patientId));
  ...
  const addNewPayment = async (e) => {
    ...
    try {
       if(!comment || !date){
        ...
        const data = await addDoc(collection(db, 'eshsessions'), paymentData);
  ...
  const deleteSession = async (sessionId) => {
    try {
      ...
      await deleteDoc(doc(db, 'eshsessions',sessionId));
